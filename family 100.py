...
def terminate_error(error_description):
    if messagebox.showerror("FAMILY 100 ERROR", error_description):
        sys.exit()
...
filename = filedialog.askopenfilename(initialdir=current_dir, title="Pilih file data", filetypes=(("csv files", "*.csv"), ("all files", "*.*")))

if filename == "":
    terminate_error("Tidak ada file yang dipilih")

if filename[-4:] != ".csv":
    terminate_error("Format file salah, pasti .csv")
...
with open(filename, "r+") as f:
    ...
    for j, line in enumerate(lines):
        ...
        if len(line) > 14:
            terminate_error(f"Setiap ronde harus memiliki paling banyak 7 jawaban, {line} memiliki {len(line)//2}")
        ...
        for i in range(0, len(line), 2):
            ...
            if len(round_answer) > 16:
                terminate_error(f"Jawaban {round_answer} di baris {i-1} terlalu panjang")

            # Check if the  is valid
            points = line[i + 1]
            if not points.isdigit() or int(points) not in range(100):
                print(line[i])
                terminate_error(f"Point {points} di baris {j+1} tidak valid")
...
pygame.display.set_caption("Family 100 Display")
programIcon = pygame.image.load("family 100.ico")
pygame.display.set_icon(programIcon)

# Create the second window
window1 = tkinter.Tk()
window1.title("Family 100 - Game Control")
window1.iconbitmap("family 100.ico")
...
for i, round_answers in enumerate(game1.answers):
    ...
    round_button = tkinter.Button(round_tab, text="Ronde Dimulai", command=lambda round=i: game1.round_init(round))
    round_button.grid(row=0, column=2, sticky="ew")

    l_start_button = tkinter.Button(round_tab, text="Team 1", command=lambda: game1.set_starting_team("L"))
    l_start_button.grid(row=1, column=1, sticky="ew")

    r_start_button = tkinter.Button(round_tab, text="Team 2", command=lambda: game1.set_starting_team("R"))
    r_start_button.grid(row=1, column=3, sticky="ew")

    l_strike_button = tkinter.Button(round_tab, text="Salah", command=lambda: game1.incorrect_answer("L"))
    l_strike_button.grid(row=2, column=1, sticky="ew")

    r_strike_button = tkinter.Button(round_tab, text="Salah", command=lambda: game1.incorrect_answer("R"))
...
score_button = tkinter.Button(score_tab, text="Tunjukkan hasil", command=game1.show_scores)
score_button.pack()
tabControl.add(score_tab, text="Tanda baca")
...
for w in range(5):
    for t in range(2):
        ...
        final_button = tkinter.Button(final_tab, text="Melihat", command=lambda r=w, c=t, answer_txt=input_answer, point_txt=input_points: game1.show_final_answer(answer_txt, point_txt, r, c))
        input_answer.grid(row=w + 1, column=t * 3)
        input_points.grid(row=w + 1, column=t * 3 + 1)
        final_button.grid(row=w + 1, column=t * 3 + 2)

startfinal_button = tkinter.Button(final_tab, text="Mulai babak bonus", command=game1.init_final_round)
startfinal_button.grid(row=0, column=2)

doubled_answer = tkinter.Button(final_tab, text="Double", command=lambda: pygame.mixer.Sound.play(dubel_sound))
...
tabControl.add(final_tab, text="Babak Bonus")

# Create SFX tab
sfx_tab = ttk.Frame(tabControl)

button1 = tkinter.Button(sfx_tab, text="Bravo", command=lambda: pygame.mixer.Sound.play(bravo_sound))
button1.pack()
...
while True:
    ...
    # Set the font
    myfont = pygame.font.Font("family 100.ttf", font_height)
